...
class CitationGraph:
    ...
    def fetch_paper_data_batch(self, paper_ids):
        """
        Fetch references and citation counts for a list of ArXiv IDs using Semantic Scholar Batch API.
        """
        ...
        params = {"fields": "paperId,externalIds,title,year,citationCount,references.paperId,references.title,references.externalIds"}
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Error fetching batch data: {response.status_code} - {response.text}")
                return []
    ...
    def build_graph(self, top_papers_metadata):
        ...
        for paper in top_papers_metadata:
            ...
            for ref in references:
                ...
                # Case A: Citing another paper in our top results
                if ref_s2_id in s2_to_arxiv:
                    G.add_edge(current_arxiv, s2_to_arxiv[ref_s2_id])
                else:
                    # Case B: Counting shared ancestors for potential bridging
